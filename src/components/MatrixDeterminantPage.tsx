    ...
    <Box w="full" p={{ base: '24px 20px', md: '40px' }} textAlign="left">
      ...
      {detail && matrix && (
        <Box mt="32px">
          ...
          <Box mt="16px" maxH="440px" overflow="auto" borderWidth="1px" borderTopWidth="2px" borderBottomWidth="3px" borderColor="var(--border)" borderRadius="12px"><Table.Root size="sm" variant="line" striped><Table.Header bg="var(--accent-bg)" borderBottomWidth="2px" borderColor="var(--accent)"><Table.Row><Table.ColumnHeader>j</Table.ColumnHeader><Table.ColumnHeader>Sign</Table.ColumnHeader><Table.ColumnHeader>a(0,j)</Table.ColumnHeader><Table.ColumnHeader>Minor M(0,j)</Table.ColumnHeader><Table.ColumnHeader>det(M)</Table.ColumnHeader><Table.ColumnHeader>Cofactor</Table.ColumnHeader><Table.ColumnHeader>Term</Table.ColumnHeader></Table.Row></Table.Header><Table.Body>{detail.terms.map((term, index) => <Table.Row key={term.j} borderBottomWidth={index === detail.terms.length - 1 ? '2px' : undefined} borderColor={index === detail.terms.length - 1 ? 'var(--accent)' : undefined}><Table.Cell>{term.j}</Table.Cell><Table.Cell fontFamily="mono">{term.sign === 1 ? '+' : '−'}</Table.Cell><Table.Cell>{term.a0j}</Table.Cell><Table.Cell><MathMatrix data={term.minor} compact /></Table.Cell><Table.Cell fontFamily="mono">{term.minorDetRaw} ≡ {term.minorDet}</Table.Cell><Table.Cell fontFamily="mono">{term.sign === 1 ? '' : '−'}({term.minorDetRaw}) ≡ {term.cofactor}</Table.Cell><Table.Cell color="var(--accent)" fontFamily="mono" fontWeight="bold">{term.sign === 1 ? '' : '−'}{term.a0j} × ({term.minorDetRaw}) ≡ {term.product}</Table.Cell></Table.Row>)}</Table.Body></Table.Root></Box>
